...
public class MultiRectAreaOp {
    ...
    public static int[] checkBufSize(int[] buf, int capacity) {
        if (buf[0] + capacity >= buf.Length) {
            ...
            java.lang.SystemJ.arraycopy(buf, 0, tmp, 0, buf[0]);
            buf = tmp;
        }
        buf[0] += capacity;
        return buf;
    ...
    internal class Region {
        ...
        void addActive(int index) {
            ...
            while(i < length) {
                if (region[index] < active[i]) {
                    // Insert
                    java.lang.SystemJ.arraycopy(active, i, active, i + 4, length - i);
                    length = i;
                    break;
                }
                i += 4;
            }
            java.lang.SystemJ.arraycopy(region, index, active, length, 4);

        }

        internal void findActive(int top, int bottom) {
        ...
        internal void deleteActive(int bottom) {
            int length = active[0];
            for(int i = 1; i < length;) {
                if (active[i + 3] == bottom) {
                    length -= 4;
                    if (i < length) {
                        java.lang.SystemJ.arraycopy(active, i + 4, active, i, length - i);
                    }
                } else {
                     i += 4;
                }
        ...
        internal void createLevel(int[] level) {
            int levelCount = 1;
            int topIndex = 1;
            int i = 1;
            while(i < region[0]) {
                ...
                bool addTopLoop = true;
                while (addTopLoop) {
                    bool break2AddTop = false;
                    bool breakAddTopLoop = false;
                    while(j < levelCount) {
                        if (level[j] == top) {
                            break2AddTop = true;
                        }
                        if (! break2AddTop) {
                            if (level[j] > top) {
                                java.lang.SystemJ.arraycopy(level, j, level, j + 1, levelCount - j);
                                breakAddTopLoop = true;
                            }
                            if (!breakAddTopLoop) {
                                j++;
                            }
                    ...
                    if (! break2AddTop && !breakAddTopLoop) {
                        level[j] = top;
                        levelCount++;
                        topIndex = j;
                    }
                ...
                bool goToAddBottom = true;
                bool stopLoop = false;
                while (goToAddBottom && !stopLoop)
                {
                    goToAddBottom = false;
                    while(j < levelCount) {
                        if (level[j] == bottom) {
        ...
        internal static void sortOrdered(int[] src1, int[] src2, int[] dst) {
            int length1 = src1[0];
            int length2 = src2[0];
            int count = 1;
            int i1 = 1;
    ...
    internal class Union {
        ...
        void check(Region d, int index, bool t) {
            int x1 = d.active[index];
            int x2 = d.active[index + 2];
            // Top
            if (d.active[index + 1] < top) {
        ...
        int[] addVerRegion(int[] top, int[] bottom) {
            ...
            dst[0] = length;
            java.lang.SystemJ.arraycopy(top, 1, dst, 1, top[0] - 1);
            java.lang.SystemJ.arraycopy(bottom, 1, dst, top[0], bottom[0] - 1);
            return dst;
        }
        ...
        int[] addHorRegion(int[] left, int[] right) {
            ...
            while(true) {
                ...
                if (index1 >= count1) {
                    java.lang.SystemJ.arraycopy(right, index2, dst, count, count2 - index2);
                    count += count2 - index2;
                    break;
                }
                if (index2 >= count2) {
                    java.lang.SystemJ.arraycopy(left, index1, dst, count, count1 - index1);
                    count += count1 - index1;
                    break;
                }
                ...
                if (top1 < top2) {
                    ...
                    java.lang.SystemJ.arraycopy(left, pos1, dst, count, index1 - pos1);
                    count += index1 - pos1;
                    continue;
                }
                ...
                if (top1 > top2) {
                    ...
                    java.lang.SystemJ.arraycopy(right, pos2, dst, count, index2 - pos2);
                    count += index2 - pos2;
                    continue;
                }
