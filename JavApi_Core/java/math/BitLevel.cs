...
namespace biz.ritter.javapi.math
{
    ...
    class BitLevel {
        ...
        internal static void shiftLeft(int []result, int []source, int intCount, int count) {
            if (count == 0) {
                java.lang.SystemJ.arraycopy(source, 0, result, intCount, result.Length
                        - intCount);
            } else {
                ...
                for (int i = result.Length - 1; i > intCount; i--) {
                    result[i] |= java.dotnet.lang.Operator.shiftRightUnsignet(source[i - intCount - 1], rightShiftCount);
        ...
        internal static void shiftLeftOneBit(int []result, int []source, int srcLen) {
            ...
            for (int i = 0; i < srcLen; i++) {
                ...
                carry = java.dotnet.lang.Operator.shiftRightUnsignet(val,31);
            }
            if (carry != 0) {
                result[srcLen] = carry;
            }
        ...
        internal static bool shiftRight(int []result, int resultLen, int []source,
            ...
            for (i = 0; i < intCount; i++)
                allZero &= source[i] == 0;
            if (count == 0) {
                java.lang.SystemJ.arraycopy(source, intCount, result, 0, resultLen);
                i = resultLen;
            } else {
                ...
                for (i = 0; i < resultLen - 1; i++) {
                    result[i] = ( java.dotnet.lang.Operator.shiftRightUnsignet(source[i + intCount], count) )
