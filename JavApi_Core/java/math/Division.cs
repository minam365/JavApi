...
namespace biz.ritter.javapi.math
{
    ...
    internal class Division {
         ...
         * @return remainder
         */
        internal static int divideArrayByInt(int []dest, int []src, int srcLength,
                int divisor) {
            ...
            for (int i = srcLength - 1; i >= 0; i--) {
                ...
                if (temp >= 0) {
                     ...
                     */
                    long aPos = java.dotnet.lang.Operator.shiftRightUnsignet(temp , 1);
                    long bPos = java.dotnet.lang.Operator.shiftRightUnsignet(divisor , 1);
                    quot = aPos / bPos;
                    rem = aPos % bPos;
        ...
        internal static long divideLongByInt(long a, int b) {
            ...
            if (a >= 0) {
                 ...
                 */
                long aPos = java.dotnet.lang.Operator.shiftRightUnsignet(a, 1);
                long bPos = java.dotnet.lang.Operator.shiftRightUnsignet(b, 1);
                quot = aPos / bPos;
                rem = aPos % bPos;
                // double the remainder and add 1 if a is odd
                rem = (rem << 1) + (a & 1);
        ...
        internal static int multiplyAndSubtract(int[] a, int start, int[] b, int bLen, int c) {
            ...
            for (int i = 0; i < bLen; i++) {
                ...
                carry1 >>=  32; // -1 or 0
                carry0 = java.dotnet.lang.Operator.shiftRightUnsignet(carry0,32);
            }
        ...
        internal static long gcdBinary(long op1, long op2) {
            // PRE: (op1 > 0) and (op2 > 0)
            int lsb1 = java.lang.Long.numberOfTrailingZeros(op1);
            int lsb2 = java.lang.Long.numberOfTrailingZeros(op2);
            int pow2Count = java.lang.Math.min(lsb1, lsb2);

            if (lsb1 != 0) {
                op1 = java.dotnet.lang.Operator.shiftRightUnsignet(op1,lsb1);
            }
            if (lsb2 != 0) {
                op2 = java.dotnet.lang.Operator.shiftRightUnsignet(op2,lsb2);
            }
            do {
                if (op1 >= op2) {
                    op1 -= op2;
                    op1 = java.dotnet.lang.Operator.shiftRightUnsignet(op1,java.lang.Long.numberOfTrailingZeros(op1));
                } else {
                    op2 -= op1;
                    op2 = java.dotnet.lang.Operator.shiftRightUnsignet(op2,java.lang.Long.numberOfTrailingZeros(op2));
                }
            } while (op1 != 0);
        ...
        private static void monReduction(int[] res, BigInteger modulus, int n2) {
            ...
            for (int i = 0; i < modulusLen; i++){
                ...
                for(int j = 0; j < modulusLen; j++){
                    ...
                    innnerCarry = java.dotnet.lang.Operator.shiftRightUnsignet(innnerCarry, 32);
                }

                outerCarry += (res[i+modulusLen] & 0xFFFFFFFFL) + innnerCarry;
                res[i+modulusLen] = (int) outerCarry;
                outerCarry = java.dotnet.lang.Operator.shiftRightUnsignet(outerCarry, 32);
            }
        ...
        internal static BigInteger monPro(BigInteger a, BigInteger b, BigInteger modulus, int n2) {
            ...
            Multiplication.multArraysPAP(a.digits, java.lang.Math.min(modulusLen, a.numberLength),
                                          b.digits, java.lang.Math.min(modulusLen, b.numberLength), res);
