    ...
    public abstract class CharsetDecoder {
        ...
        public CharsetDecoder replaceWith(String newReplacement) {
            if (null == newReplacement || newReplacement.length() == 0) {
                ...
                throw new java.lang.IllegalArgumentException("Replacement string cannot be null or empty."); //$NON-NLS-1$
            }
            if (newReplacement.length() > maxChars) {
                // niochar.07=Replacement string's length cannot be larger than max
                // characters per byte.
                throw new java.lang.IllegalArgumentException("Replacement string's length cannot be larger than max characters per byte."); //$NON-NLS-1$
            }
