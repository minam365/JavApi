 ...
 */
using System; // Load extensions methods
...
namespace biz.ritter.javapi.util.zip
{
    ...
    public class CRC32 : HashAlgorithm, java.util.zip.Checksum {
        ...
        protected internal long tbytes;
        ...
        {
            checksum = 0xffffffff; // recompute for full byte array
            int off = 0;
            int len = this.content.Length;
            while (len >= 4)
            {
                checksum = T4[((checksum) ^ content[off]) & 0xff]
                    ^ T3[((dotnet.lang.Operator.shiftRightUnsignet(checksum, 8)) ^ content[off + 1]) & 0xff]
                    ^ T2[((dotnet.lang.Operator.shiftRightUnsignet(checksum, 16)) ^ content[off + 2]) & 0xff]
                    ^ T1[((dotnet.lang.Operator.shiftRightUnsignet(checksum, 24)) ^ content[off + 3]) & 0xff];
                off += 4;
                len -= 4;
            }
            while (len > 0)
            {
                checksum = (dotnet.lang.Operator.shiftRightUnsignet(checksum, 8)) ^ T1[(checksum ^ content[off++]) & 0xff];
                len--;
            }
        }
