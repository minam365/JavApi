...
namespace biz.ritter.javapi.io
{
	...
	public class DataOutputStream : FilterOutputStream, DataOutput {
	    ...
	    public int size() {
	        if (written < 0) {
	            written = java.lang.Integer.MAX_VALUE;
	        }
	        return written;
	    }
	    ...
	    public override void write(byte[] buffer, int offset, int count){// throws IOException {
	        if (buffer == null) {
	            throw new java.lang.NullPointerException("buffer is null"); //$NON-NLS-1$
	        }
	        outJ.write(buffer, offset, count);
	        written += count;
	    }
	     ...
	     */
	    public void writeBoolean(bool val) {//throws IOException {
	        outJ.write(val ? 1 : 0);
	        written++;
	    }
	    ...
	    internal int writeLongToBuffer(long val,
	    ...
	    internal int writeShortToBuffer(int val,
	                           byte[] buffer, int offset) {//throws IOException {
	        buffer[offset++] = (byte) (val >> 8);
	        buffer[offset++] = (byte) val;
	        return offset;
	    ...
	    public  void writeUTF(String str) {//throws IOException {
	        ...
	        if (utfCount > 65535) {
	            throw new UTFDataFormatException("String is too long"); //$NON-NLS-1$
	        }
