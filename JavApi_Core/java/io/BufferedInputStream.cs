...
namespace biz.ritter.javapi.io
{
    ...
    public class BufferedInputStream : FilterInputStream {
         ...
         */
        public BufferedInputStream(InputStream inJ) : base (inJ){
            buf = new byte[8192];
        }
         ...
         *             if {@code size &lt; 0}.
         */
        public BufferedInputStream(InputStream inJ, int size) : base (inJ) {
            if (size <= 0) {
        ...
        {
            lock (this) {
                InputStream localIn = inJ; // 'inJ' could be invalidated by close()
